...
export type StoredConfig = {
  ...
  /** Configuration for the codebase graph feature */
  graph?: {
    enabled?: boolean;
    // Add other graph-related settings here if needed, e.g.,
    // autoIndex?: boolean;
...
export type AppConfig = {
  ...
  /** Configuration for the codebase graph feature */
  graph?: {
    enabled: boolean; // This will reflect the final decision (CLI flag or config)
    // Add other graph-related settings here
  };
};
...
export type LoadConfigOptions = {
  ...
  /** Value of the --graph CLI flag */
  graphEnabledFlag?: boolean;
};
...
export const loadConfig = (
  ...
  // Load graph settings, providing defaults and prioritizing the CLI flag
  config.graph = {
    enabled: options.graphEnabledFlag ?? storedConfig.graph?.enabled ?? false,
    // Add defaults for other graph settings here
  };
...
export const saveConfig = (
  ...
  // Add graph settings if they exist - save the config file value, not the runtime flag value
  if (config.graph) {
    // Load the stored config again to get the value *without* the CLI flag override
    let storedGraphEnabled = false;
    try {
      if (existsSync(actualConfigPath)) {
        const raw = readFileSync(actualConfigPath, "utf-8");
        const ext = extname(actualConfigPath).toLowerCase();
        let storedConf: StoredConfig = {};
        if (ext === ".yaml" || ext === ".yml") {
          storedConf = loadYaml(raw) as unknown as StoredConfig;
        } else {
          storedConf = JSON.parse(raw);
        }
        storedGraphEnabled = storedConf.graph?.enabled ?? false;
      }
    } catch { /* ignore */ }

    configToSave.graph = {
      enabled: storedGraphEnabled,
      // Add other graph settings to save here
    };
  }
