...
// Import graph indexing function
import { buildOrUpdateGraph } from "./graph/index.js";
// Import graph visualization function
import { generateGraphVisualization } from "./graph/visualize.js";
...
const cli = meow(
  ...
  Usage
    ...
    $ codex index                     Build or update the codebase graph index
    $ codex visualize [output-file]   Generate static HTML visualization of the graph
  ...
  Experimental options
    ...
    --graph                    Enable codebase graph context retrieval (experimental)
...
`,
  {
    ...
    flags: {
      ...
      // Experimental graph context retrieval
      graph: {
        type: "boolean",
        description: "Enable codebase graph context retrieval (experimental)",
      },
    },
  },
);
...
// Moved config loading here, before the 'index' or 'visualize' command check
const fullContextMode = Boolean(cli.flags.fullContext);
const graphModeEnabled = Boolean(cli.flags.graph); // Check if --graph flag is present
...
let config = loadConfig(undefined, undefined, {
  ...
  graphEnabledFlag: graphModeEnabled, // Pass the flag value to loadConfig
});

// Handle 'index' command
if (cli.input[0] === "index") {
  (async () => {
    try {
      // eslint-disable-next-line no-console
      console.log("Building codebase graph index...");
      await buildOrUpdateGraph(process.cwd(), config);
      // eslint-disable-next-line no-console
  ...
  // Keep the process alive while indexing runs
} else if (cli.input[0] === "visualize") {
  ...
  (async () => {
    try {
      ...
      const outputFileName = cli.input[1] || "codex-graph.html";
      const outputHtmlPath = path.resolve(projectRoot, outputFileName);
      await generateGraphVisualization(projectRoot, outputHtmlPath);
      // Message is printed by generateGraphVisualization on success
      process.exit(0);
    } catch (error) {
      // eslint-disable-next-line no-console
  ...
  // Existing logic for other commands or interactive mode

  // For --help, show help and exit.
  if (cli.flags.help) {
    cli.showHelp();
    process.exit(0); // Exit after showing help if not indexing
  }
  ...
  config = {
    ...
    // Ensure graph config is correctly merged, prioritizing the CLI flag
    graph: {
      ...config.graph, // Keep other graph settings from config file
      enabled: graphModeEnabled || (config.graph?.enabled ?? false), // Prioritize flag
    },
  };
  ...
  const approvalPolicy: ApprovalPolicy =
    cli.flags.dangerouslyAutoApproveEverything
      ? AutoApprovalMode.DANGEROUSLY_AUTO_APPROVE_EVERYTHING
      : cli.flags.fullAuto || cli.flags.approvalMode === "full-auto"
        ? AutoApprovalMode.FULL_AUTO
        : cli.flags.autoEdit || cli.flags.approvalMode === "auto-edit"
          ? AutoApprovalMode.AUTO_EDIT
  ...
  const instance = render(
    <App
      prompt={prompt}
      config={config} // Pass the final config, including graph status
      rollout={rollout}
      imagePaths={imagePaths}
  ...
  async function runQuietMode({
    ...
    const agent = new AgentLoop({
      ...
      graphEnabled: config.graph?.enabled ?? false, // Pass graph status
...
} // End of the 'else' block for non-index/non-visualize commands